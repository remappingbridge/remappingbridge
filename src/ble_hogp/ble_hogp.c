...
static blu2usb_ble_hogp_report_state_t *find_report(blu2usb_ble_hogp_parser_t *parser,
                                                     uint8_t report_id,
                                                     bool create)
{
    ...
    if (!create || parser->report_count >= BLU2USB_BLE_HOGP_MAX_REPORTS) return NULL;
    blu2usb_ble_hogp_report_state_t *report = &parser->reports[parser->report_count++];
    memset(report, 0, sizeof(*report));
    report->report_id = report_id;
    return report;
}

static const blu2usb_ble_hogp_report_state_t *find_report_const(const blu2usb_ble_hogp_parser_t *parser,
                                                                 uint8_t report_id)
{
...
static bool add_field(blu2usb_ble_hogp_parser_t *parser,
                      uint8_t report_id,
                      ...
                      bool signed_value)
{
    if (parser->field_count >= BLU2USB_BLE_HOGP_MAX_FIELDS || bit_size == 0u || bit_size > 32u) return false;
    blu2usb_ble_hogp_field_t *field = &parser->fields[parser->field_count++];
...
static bool classify_and_add(blu2usb_ble_hogp_parser_t *parser,
                             ...
                             uint32_t input_flags)
{
    ...
    if (page == HID_USAGE_PAGE_BUTTON && code >= 1u && code <= BLU2USB_MOUSE_BUTTON_COUNT) {
        return add_field(parser, globals->report_id, BLU2USB_BLE_HOGP_FIELD_BUTTON,
                         bit_offset, globals->report_size, (uint8_t)(code - 1u), false);
    }
    if (!relative) return true;

    if (page == HID_USAGE_PAGE_GENERIC_DESKTOP) {
        if (code == HID_USAGE_X) return add_field(parser, globals->report_id, BLU2USB_BLE_HOGP_FIELD_X,
                                                   bit_offset, globals->report_size, 0u, globals->logical_minimum < 0);
        if (code == HID_USAGE_Y) return add_field(parser, globals->report_id, BLU2USB_BLE_HOGP_FIELD_Y,
                                                   bit_offset, globals->report_size, 0u, globals->logical_minimum < 0);
        if (code == HID_USAGE_WHEEL) return add_field(parser, globals->report_id, BLU2USB_BLE_HOGP_FIELD_WHEEL,
                                                       bit_offset, globals->report_size, 0u, globals->logical_minimum < 0);
    }
    if (page == HID_USAGE_PAGE_CONSUMER && code == HID_USAGE_AC_PAN) {
        return add_field(parser, globals->report_id, BLU2USB_BLE_HOGP_FIELD_PAN,
                         bit_offset, globals->report_size, 0u, globals->logical_minimum < 0);
    }
    return true;
}
...
{
    if (parser == NULL || descriptor == NULL || descriptor_len == 0u || !blu2usb_hid_source_is_valid(source)) return false;
    ...
    while (position < descriptor_len) {
        ...
        if (item_tag == HID_MAIN_INPUT) {
            blu2usb_ble_hogp_report_state_t *report = find_report(parser, globals.report_id, true);
                                               ...
                                               size_t *payload_len)
{
    ...
    const blu2usb_ble_hogp_report_state_t *state = find_report_const(parser, report_id);
...
static bool emit_button_deltas(blu2usb_ble_hogp_parser_t *parser,
                               uint8_t old_mask,
                               uint8_t new_mask,
                               ...
                               void *context)
{
    ...
    for (uint8_t button = 0u; button < BLU2USB_MOUSE_BUTTON_COUNT; ++button) {
        ...
        event.data.button.button = (blu2usb_mouse_button_t)button;
...
static bool parse_payload(blu2usb_ble_hogp_parser_t *parser,
                          uint8_t report_id,
                          const uint8_t *report,
                          size_t report_len,
                          blu2usb_ble_hogp_emit_fn emit,
                          void *context)
{
    blu2usb_ble_hogp_report_state_t *report_state = find_report(parser, report_id, false);
    ...
    for (size_t index = 0u; index < parser->field_count; ++index) {
        const blu2usb_ble_hogp_field_t *field = &parser->fields[index];
        ...
        switch (field->kind) {
        case BLU2USB_BLE_HOGP_FIELD_BUTTON: {
        ...
        case BLU2USB_BLE_HOGP_FIELD_X: dx += value; break;
        case BLU2USB_BLE_HOGP_FIELD_Y: dy += value; break;
        case BLU2USB_BLE_HOGP_FIELD_WHEEL: wheel += value; break;
        case BLU2USB_BLE_HOGP_FIELD_PAN: pan += value; break;
        }
    }
                                           ...
                                           void *context)
{
    ...
    if (!blu2usb_ble_hogp_parser_normalize_report(parser, report_id, report, report_len, &payload, &payload_len)) return false;
...
{
    if (message == NULL || event == NULL || message->channel != BLU2USB_BLE_HOGP_RUNTIME_CHANNEL) return false;
    memset(event, 0, sizeof(*event));
    switch (message->type) {
    case BLU2USB_BLE_HOGP_MESSAGE_CONNECTED:
        if (message->length != 0u) return false;
        event->type = BLU2USB_BLE_HOGP_EVENT_CONNECTED;
        return true;
    case BLU2USB_BLE_HOGP_MESSAGE_DISCONNECTED:
        if (message->length != 0u) return false;
        event->type = BLU2USB_BLE_HOGP_EVENT_DISCONNECTED;
        return true;
    case BLU2USB_BLE_HOGP_MESSAGE_MOUSE:
        if (message->length != sizeof(blu2usb_canonical_mouse_event_t)) return false;
